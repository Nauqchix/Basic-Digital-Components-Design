module CLA_ADD_SUB_32_TB;
  ...
  CLA_ADD_SUB_32_ARCH_MODEL cla_add_sub_arch_inst(.Y(Y_ARCH), .CO(CO_ARCH), .A(A), .B(B), .SnA(SnA));
  
  CLA_ADD_SUB_32_BEH_MODEL  cla_add_sub_beh_inst (.Y(Y_BEH), .CO(CO_BEH), .A(A), .B(B), .SnA(SnA));
